...
class DadosTelhado(Base):
    ...
    orientacao: Mapped[OrientacaoTelhado | None] = mapped_column(
        Enum(OrientacaoTelhado, name="orientacao_telhado"), nullable=True
    )
    tipo_conexao: Mapped[TipoConexao | None] = mapped_column(
        Enum(TipoConexao, name="tipo_conexao"), nullable=True
    )
...
class Resultado(Base):
    ...
    quantidade_paineis: Mapped[int | None] = mapped_column(Integer, nullable=True)
    potencia_sistema_kwp: Mapped[Decimal | None] = mapped_column(Numeric(8, 2))
